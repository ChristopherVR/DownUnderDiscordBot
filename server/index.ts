...
const localesDir = path.resolve(projectRoot, 'shared', 'src', 'localization', 'locales');

const serverLog = createLogger('server');

// Prefer a user override if present; otherwise use the bundled binary.
process.env.FFMPEG_PATH = 'D:\\youtube-dl\\ffmpeg.exe'; // process.env.FFMPEG_PATH || ffmpeg.path;
...
async function main() {
  ...
  app.get('index', (_req, res) => res.sendFile(path.join(clientDist, 'index.html')));
