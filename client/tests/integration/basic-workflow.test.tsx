...
describe('Basic Application Workflow', () => {
  ...
  beforeEach(() => {
    ...
    apiMocks.state.getState.mockResolvedValue(globalState);
    apiMocks.getGlobalState.mockResolvedValue(globalState);
  ...
  it('renders dashboard layout after initial data load', async () => {
    renderApp();
  ...
  it('navigates between primary routes', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('heading', { name: 'musicPlayer.title', level: 1 })).toBeInTheDocument();
    });
    ...
    expect(screen.getByRole('button', { name: 'commandInvocation.form.execute' })).toBeInTheDocument();
  });

  it('shows resilient layout when API calls fail', async () => {
    apiMocks.state.getState.mockRejectedValueOnce(new Error('network'));
    apiMocks.music.getPlayerState.mockRejectedValueOnce(new Error('network'));
